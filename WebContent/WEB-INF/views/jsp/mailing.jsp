...
<script>//Para cambiar el enlace de compartir por Whatsapp en p�ginas de detalle
	 $(document).ready(function() {
		...
		<!-- Contenido -->
			...
			<div class="adminlist">
		<h2>Report d'una campanya</h2>			
		</div>
		<form action="downloadMailList">
		<div class="col-sm-12 report">
			<label>ID: </label><br>
			<select name="idCampagna">
				<option value="0">[Selecciona]</option>

				<%
					List<String> campagnas = (List<String>) request.getAttribute("campagnas");
					

					for (int i = 0; i < campagnas.size(); i++) {
						String currentCampagna = campagnas.get(i);
				%>
				<option value=<%=currentCampagna%>><%=currentCampagna%></option>
				<%
					}
				%>
		...
		<div class="col-sm-12 report">
				...
				<input type= "submit" onclick="validate();" value="Descarregar lista" class="btn bgRed white margin_top_xs">
				<!-- <a href="/downloadCSV" class = "btn bgRed white margin_top_xs">Descarregar report</a>
				 -->
		</div>	
		</form>	
			<div class="col-sm-12 report">
...
<script type="text/javascript">
	document.getElementByName('idCampagna')
	function validate()
	{
	 var ddl = document.getElementByName("idCampagna");
	 var selectedValue = ddl.options[ddl.selectedIndex].value;
	    if (selectedValue === '0')
	   {
	    alert("Please select id of campagna");
	   }
	}
