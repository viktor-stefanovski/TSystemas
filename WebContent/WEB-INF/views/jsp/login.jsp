...
<script>//Para cambiar el enlace de compartir por Whatsapp en p�ginas de detalle
	 $(document).ready(function() {
		...
		<!-- Contenido -->
			...
			<form method="GET" action="admin" class="form-signin">
			        <h2 class="form-heading">Inicieu Sessió</h2>
			
			        <div class="form-group-login ${error != null ? 'has-error' : ''}">
			            <span>${message}</span>
			            <input name="username" type="text" class="form-control" placeholder="Usuari"
			                   autofocus="true"/>
			            <input name="password" type="password" class="form-control" placeholder="Contrasenya" size="35"/>
			            <span>${error}</span>
						<div class="accedir">
			           	 	<input type = "submit" class="btn bgRed white margin_top_xs" value="Accediu">
			            </div>
			        </div>
